...
void ashenvale::editor::console::render()
{
    ...
    if (renderer::device::g_infoQueue)
    {
        ...
        for (UINT64 i = 0; i < numMessages; i++)
        {
            ...
            {
            case D3D11_MESSAGE_SEVERITY_CORRUPTION: color = ImVec4(1.0f, 0.0f, 0.0f, 1.0f); break; // bright red
            case D3D11_MESSAGE_SEVERITY_ERROR:      color = ImVec4(1.0f, 0.4f, 0.4f, 1.0f); break; // red
            case D3D11_MESSAGE_SEVERITY_WARNING:    color = ImVec4(1.0f, 1.0f, 0.0f, 1.0f); break; // yellow
            case D3D11_MESSAGE_SEVERITY_INFO:       color = ImVec4(0.5f, 0.5f, 1.0f, 1.0f); break; // blue-ish
            case D3D11_MESSAGE_SEVERITY_MESSAGE:    color = ImVec4(1.0f, 1.0f, 1.0f, 1.0f); break; // white
            default:                                color = ImVec4(0.7f, 0.7f, 0.7f, 1.0f); break; // gray
            }

            std::string severity;
            switch (message->Severity)
            {
            case D3D11_MESSAGE_SEVERITY_CORRUPTION: severity = "[D3D11 CORRUPTION] "; break;
            case D3D11_MESSAGE_SEVERITY_ERROR:      severity = "[D3D11 ERROR] "; break;
            case D3D11_MESSAGE_SEVERITY_WARNING:    severity = "[D3D11 WARNING] "; break;
            case D3D11_MESSAGE_SEVERITY_INFO:       severity = "[D3D11 INFO] "; break;
            case D3D11_MESSAGE_SEVERITY_MESSAGE:    severity = "[D3D11 MESSAGE] "; break;
            default: severity = "[D3D11 UNKNOWN] "; break;
            }
